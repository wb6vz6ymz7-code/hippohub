...
local VIRTUAL_KEY_MAP = {
    [Enum.KeyCode.One] = "1", [Enum.KeyCode.Two] = "2", [Enum.KeyCode.Three] = "3",
    [Enum.KeyCode.Four] = "4", [Enum.KeyCode.Five] = "5", [Enum.KeyCode.Six] = "6",
    [Enum.KeyCode.Seven] = "7", [Enum.KeyCode.Eight] = "8", [Enum.KeyCode.Nine] = "9",
    [Enum.KeyCode.Zero] = "0", [Enum.KeyCode.Space] = " ", [Enum.KeyCode.R] = "r",
    [Enum.KeyCode.E] = "e", [Enum.KeyCode.F] = "f", [Enum.KeyCode.Q] = "q",
    [Enum.KeyCode.T] = "t", [Enum.KeyCode.G] = "g", [Enum.KeyCode.V] = "v",
    [Enum.KeyCode.C] = "c", [Enum.KeyCode.X] = "X", [Enum.KeyCode.Z] = "z",
    [Enum.KeyCode.B] = "b", [Enum.KeyCode.H] = "h", [Enum.KeyCode.Y] = "Y",
    [Enum.KeyCode.U] = "u", [Enum.KeyCode.I] = "i", [Enum.KeyCode.O] = "o",
    [Enum.KeyCode.P] = "p", [Enum.KeyCode.L] = "l", [Enum.KeyCode.K] = "k",
    [Enum.KeyCode.J] = "j", [Enum.KeyCode.M] = "m", [Enum.KeyCode.N] = "n",
    [Enum.KeyCode.W] = "w", [Enum.KeyCode.A] = "a", [Enum.KeyCode.S] = "s",
    [Enum.KeyCode.D] = "d", [Enum.KeyCode.Return] = "return",
    [Enum.KeyCode.LeftShift] = "shift", [Enum.KeyCode.LeftControl] = "ctrl",
}
local VIRTUAL_TOUCH_KEYS = {
    ["1"] = 0x31, ["2"] = 0x32, ["3"] = 0x33,
    ["r"] = 0x52, ["e"] = 0x45, ["f"] = 0x46, ["q"] = 0x51,
    ["t"] = 0x54, ["g"] = 0x47, ["space"] = 0x20,
    ["shift"] = 0xA0, ["ctrl"] = 0xA2, ["return"] = 0x0D,
}
local MOBILE_TOUCH_STATE = {
    touching = false,
...
UserInputService.TouchStarted:Connect(function(touch, gameProcessed)
    ...
    MOBILE_TOUCH_STATE.touching = true
    MOBILE_TOUCH_STATE.touchStartPos = touch.Position
    MOBILE_TOUCH_STATE.touchCurrentPos = touch.Position
    MOBILE_TOUCH_STATE.lastTapTime = tick()
end)
...
UserInputService.TouchEnded:Connect(function(touch, gameProcessed)
    MOBILE_TOUCH_STATE.touching = false
end)
local visibilityCache = {}
local VISIBILITY_CACHE_TTL = 0.12
local function isShootButtonPressed()
    if IS_MOBILE then
        return MOBILE_TOUCH_STATE.touching
    end
    return UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1)
end
local function getScreenPointForAim()
    if IS_MOBILE then
        if MOBILE_TOUCH_STATE.touching then
            return MOBILE_TOUCH_STATE.touchCurrentPos
        end
...
local Tabs = {
	...
	Character = Window:AddTab("角色"),
	Visuals = Window:AddTab("視覺"),
	World = Window:AddTab("世界"),
	Misc = Window:AddTab("雜項"),
	['介面設定'] = Window:AddTab('設定'),
}
...
local function instanceSafeRequire(moduleRef, timeoutSec)
    ...
    error("[Lunara] module load failed: " .. key .. " (" .. tostring(lastErr) .. ")")
end
...
-- 河馬科技 動態玻璃 UI（效能優化 + 強度可調）
getgenv().HemaGlassEnabled = false
getgenv().HemaGlassStrength = getgenv().HemaGlassStrength or 0.7 -- 0.3弱 ~ 1.0強

getgenv().HemaGlassState = getgenv().HemaGlassState or { backups = {}, created = {}, loopConn = nil }

local function hemaGlassDestroyCreated()
    local st = getgenv().HemaGlassState
    for _, obj in ipairs(st.created or {}) do
        pcall(function() if obj and obj.Parent then obj:Destroy() end end)
    end
    st.created = {}
end

local function hemaGlassRestore()
    local st = getgenv().HemaGlassState
    for obj, bak in pairs(st.backups or {}) do
        pcall(function()
            if obj and obj.Parent then
                if bak.bg ~= nil then obj.BackgroundColor3 = bak.bg end
                if bak.tr ~= nil then obj.BackgroundTransparency = bak.tr end
            end
        end)
    end
    st.backups = {}
    hemaGlassDestroyCreated()
    if st.loopConn then pcall(function() st.loopConn:Disconnect() end) st.loopConn = nil end
    if st.tabConn then pcall(function() st.tabConn:Disconnect() end) st.tabConn = nil end
end

local function hemaGetRoots()
    local roots = {}
    if not Library or not Library.ScreenGui then return roots end
    for _, obj in ipairs(Library.ScreenGui:GetDescendants()) do
        if obj:IsA("Frame") and string.lower(obj.Name) == "outer" then
    ...
    if #roots == 0 then
        for _, obj in ipairs(Library.ScreenGui:GetChildren()) do
            if obj:IsA("Frame") then
                local ok, sz = pcall(function() return obj.AbsoluteSize end)
                if ok and sz and sz.X > 350 and sz.Y > 250 then table.insert(roots, obj) end
            end
        end
    end
    return roots
...
local function hemaGlassPaintOnce()
    local st = getgenv().HemaGlassState
    if not getgenv().HemaGlassEnabled or not Library or not Library.ScreenGui then return end
    local strength = math.clamp(tonumber(getgenv().HemaGlassStrength) or 0.7, 0.25, 1)
    -- 透明度：強度越高越透
    local trOuter = 0.35 + strength * 0.35  -- 0.45 ~ 0.70
    local trMain  = 0.28 + strength * 0.30
    local trBox   = 0.22 + strength * 0.28
    local trSmall = 0.18 + strength * 0.22

    local function bak(fr)
        if not st.backups[fr] then
            st.backups[fr] = { bg = fr.BackgroundColor3, tr = fr.BackgroundTransparency }
        end
    end
    local function paint(fr, tr, col)
        if not fr or not (fr:IsA("Frame") or fr:IsA("ScrollingFrame")) then return end
        if fr.BackgroundTransparency >= 0.98 then return end
        local n = string.lower(tostring(fr.Name))
        if n:find("checkmark") or n:find("knob") or n:find("caret") then return end
        bak(fr)
        fr.BackgroundColor3 = col
        fr.BackgroundTransparency = tr
    end

    for _, root in ipairs(hemaGetRoots()) do
        paint(root, trOuter, Color3.fromRGB(8, 10, 16))
        for _, d in ipairs(root:GetDescendants()) do
            if d:IsA("Frame") or d:IsA("ScrollingFrame") then
                local ok, sz = pcall(function() return d.AbsoluteSize end)
                local area = (ok and sz) and (sz.X * sz.Y) or 0
                local n = string.lower(tostring(d.Name))
                if n == "main" or area > 100000 then
                    paint(d, trMain, Color3.fromRGB(10, 12, 18))
                elseif area > 20000 or n:find("group") or n:find("tab") or n:find("box") or n:find("holder") or n:find("section") or n:find("container") then
                    paint(d, trBox, Color3.fromRGB(12, 14, 22))
                elseif area > 800 then
                    paint(d, trSmall, Color3.fromRGB(14, 16, 24))
                end
            end
            -- 文字略加對比：深色字在玻璃上加一點描邊感（不改字色以免花）
        end
        if not root:FindFirstChild("HemaGlassStroke") then
            local stroke = Instance.new("UIStroke")
            stroke.Name = "HemaGlassStroke"
            stroke.Thickness = 2
            stroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
            stroke.Color = Color3.fromRGB(0, 200, 255)
            stroke.Parent = root
            table.insert(st.created, stroke)
        end
        if not root:FindFirstChild("HemaGlassShine") then
            local shine = Instance.new("Frame")
            shine.Name = "HemaGlassShine"
            shine.BackgroundColor3 = Color3.fromRGB(220, 230, 255)
            shine.BackgroundTransparency = 0.88
            shine.BorderSizePixel = 0
            shine.Size = UDim2.new(1, 0, 0, 48)
            shine.ZIndex = 60
            shine.Active = false
            shine.Parent = root
            ...
            sg.Parent = shine
            table.insert(st.created, shine)
        end
    end
end

local function hemaGlassApply()
    hemaGlassRestore()
    local st = getgenv().HemaGlassState
    st.backups, st.created = {}, {}
    hemaGlassPaintOnce()
    -- 只在 UI 樹有新節點時補套（比固定 0.45s 掃整樹省）
    if Library and Library.ScreenGui then
        st.tabConn = Library.ScreenGui.DescendantAdded:Connect(function(obj)
            if not getgenv().HemaGlassEnabled then return end
            if obj:IsA("Frame") or obj:IsA("ScrollingFrame") then
                task.defer(hemaGlassPaintOnce)
            end
        end)
    end
    local hue = 0
    st.loopConn = game:GetService("RunService").RenderStepped:Connect(function()
        if not getgenv().HemaGlassEnabled then return end
        hue = (hue + 0.007) % 1
        local col = Color3.fromHSV(hue, 0.85, 1)
        for _, obj in ipairs(st.created) do
            if obj and obj.Parent and obj:IsA("UIStroke") then
                obj.Color = col
            end
        end
    end)
...
getgenv().HemaGlassSetEnabled = function(on)
    getgenv().HemaGlassEnabled = on and true or false
    if getgenv().HemaGlassEnabled then
        task.defer(function() task.wait(0.05) hemaGlassApply() end)
    else
        hemaGlassRestore()
    end
end

getgenv().HemaGlassSetStrength = function(v)
    getgenv().HemaGlassStrength = math.clamp(tonumber(v) or 0.7, 0.25, 1)
    if getgenv().HemaGlassEnabled then
        hemaGlassPaintOnce()
    end
end
...
task.spawn(function()
    task.wait(1)
    if getgenv().HemaGlassEnabled then hemaGlassApply() end
end)
...
local function drawInstanceMenuCursor()
    ...
    local cam = workspace.CurrentCamera
    local mPos = cam and Vector2.new(cam.ViewportSize.X / 2, cam.ViewportSize.Y / 2) or Vector2.new(0, 0)
...
local InstanceLocaleTable = {
    english = {
        ...
        apply = "apply", apply_all = "apply to all", language = "語言",
        ...
        skin = "皮膚", wrap = "wrap", rclick_hint = "r-click weapon: remove skin",
    ...
    spanish = {
        ...
        skin = "皮膚", wrap = "wrap", rclick_hint = "clic derecho: quitar skin",
...
local function worldToScreen(worldPos, cam)
    ...
    if not ok or not onScreen or v.Z <= 0 then
        return v, false
    end
...
do
local RightGroup = Tabs.Combat:AddLeftTabbox()
local silenttab           = RightGroup:AddTab('靜默瞄準')
local silentcustomization = RightGroup:AddTab('自訂')
local AimbotGroup     = Tabs.Combat:AddRightTabbox()
local aimbotTab       = AimbotGroup:AddTab('自瞄')
local aimbotCustomTab = AimbotGroup:AddTab('自訂')
...
do
    ...
    HPlist = {
        "頭部","HumanoidRootPart","Torso","UpperTorso","LowerTorso",
...
local silentAim    = { enabled=false, hitPart="Head", fovRadius=60, autoShoot=false, followMuzzle=false, hitChance=100 }
local aimbot       = {
    enabled = false,
    masterEnabled = false,
    keyMode = "toggle",
...
local function restoreGunSoundVolumes()
    local function tryRestore(root)
        ...
        for _, d in ipairs(root:GetDescendants()) do
            if d:IsA("音效") and isRivalsGunSound(d) and not d:GetAttribute("InstanceShootSound") then
                pcall(function()
                    if d.Volume <= 0 then
                        d.Volume = 1
                    end
...
local function isRivalsGunSound(sound)
    if typeof(sound) ~= "Instance" or not sound:IsA("音效") then
        return false
    end
...
local function bindGunSoundMuteGuard(sound)
    if typeof(sound) ~= "Instance" or not sound:IsA("音效") or sound:GetAttribute("InstanceShootSound") then
        return
    end
    if not sound:GetAttribute("InstanceGunMuteBound") then
        sound:SetAttribute("InstanceGunMuteBound", true)
        sound:GetPropertyChangedSignal("音量"):Connect(function()
...
local function refreshGunSoundMute()
    ...
    if vm then
        for _, d in ipairs(vm:GetDescendants()) do
            if d:IsA("音效") or d:IsA("SoundGroup") then
                if d:IsA("音效") then
...
local function hitpartfromname(target, partName)
    ...
    if     partName == "頭部"             then return fc("頭部")
...
RunService.Heartbeat:Connect(function()
    if silentAim.enabled then
        if silentAim.autoShoot or isShootButtonPressed() then
            firesilent()
        end
    end
end)
...
task.wait(0.8)
LocalPlayer:WaitForChild("PlayerScripts", 10)
local camController
pcall(function()
...
local function isTargetVisible(targetPart, myRoot)
    if not targetPart or not targetPart.Parent then return false end
    local cacheKey = targetPart
    local cached = visibilityCache[cacheKey]
    local now = tick()
    if cached and (now - cached.t) < VISIBILITY_CACHE_TTL then
        return cached.v
    end
    local params = RaycastParams.new()
    params.FilterDescendantsInstances = {LocalPlayer.Character, targetPart.Parent}
    params.FilterType = Enum.RaycastFilterType.Blacklist
    local toTarget = targetPart.Position - myRoot.Position
    local result = workspace:Raycast(myRoot.Position, toTarget.Unit * toTarget.Magnitude, params)
    local visible = not result or (result.Instance and (result.Instance:IsDescendantOf(targetPart.Parent) or result.Instance == targetPart))
    visibilityCache[cacheKey] = {t = now, v = visible}
    return visible
end
task.spawn(function()
    while true do
        ...
        for k, v in pairs(visibilityCache) do
            if not k.Parent or (now - v.t) > VISIBILITY_CACHE_TTL * 3 then
            ...
            if part and part:IsDescendantOf(workspace) then
                ...
                if on then
                    ...
                    if dist < bestDist then
                        if aimbot.visibleOnly and myRoot then
                            if isTargetVisible(part, myRoot) then
                                bestDist = dist
                                best = part
                            end
                        else
...
local function stepAimbot(dt)
    ...
    local myHead = myChar:FindFirstChild("頭部")
    if not myHead then
        clearAimbotLock()
        return
    end
...
silenttab:AddToggle("BackshootToggle", {
    Text     = "背後射擊",
    Default  = false,
    Callback = function(val)
...
silenttab:AddToggle("AntiKatana", {
    Text     = "反武士刀",
    Default  = false,
...
silenttab:AddSlider("HitChance", {
    ...
    Rounding = 0,
    Compact  = true,
    Callback = function(val) silentAim.hitChance = val end
})
silenttab:AddDropdown("HitPartDropdown", {
    Text     = "命中部位",
    Default  = "頭部",
    Values   = HPlist,
    Callback = function(val) silentAim.hitPart = val end
})
silentcustomization:AddToggle("ShowFOV", {
    Text     = "顯示 FOV",
    Default  = false,
...
silentcustomization:AddToggle("SilentFOVFilled", {
    Text     = "FOV 填充",
    Default  = false,
...
silentcustomization:AddToggle("SilentFOVFillAnimated", {
    Text     = "動畫填充",
    Default  = false,
    Callback = function(val)
...
silentcustomization:AddSlider("FOVRadius", {
    Text     = "FOV 半徑",
    Default  = 100,
    Min      = 10,
    Max      = 750,
...
silentcustomization:AddSlider("SilentFOVOutlineThickness", {
    Text     = "外框粗細",
    Default  = 1.5,
    Min      = 0.5,
    Max      = 5,
...
silentcustomization:AddSlider("SilentFOVOutlineTransparency", {
    Text     = "外框透明度",
    Default  = 0,
    Min      = 0,
    Max      = 1,
...
silentcustomization:AddSlider("SilentFOVOutlineRotation", {
    Text     = "外框旋轉",
    Default  = 0,
    Min      = 0,
    Max      = 360,
...
silentcustomization:AddSlider("SilentFOVFillTransparency", {
    Text     = "填充透明度",
    Default  = 0.7,
    Min      = 0,
    Max      = 1,
...
silentcustomization:AddSlider("SilentFOVFillRotation", {
    Text     = "填充旋轉",
    Default  = 0,
    Min      = 0,
    Max      = 360,
...
silentcustomization:AddSlider("SilentFOVFillSpeed", {
    Text     = "填充速度",
    Default  = 1,
    Min      = 0.1,
    Max      = 10,
...
silentcustomization:AddToggle("SilentFOVSpin", {
    Text     = "FOV 旋轉",
    Default  = false,
    Callback = function(val)
...
silentcustomization:AddSlider("SilentFOVSpinSpd", {
    Text     = "旋轉速度",
    Default  = 1,
    Min      = 0.1,
    Max      = 10,
...
silentcustomization:AddToggle("SilentFOVFollowMuzzle", {
    Text     = "跟隨槍口",
    Default  = false,
...
aimbotTab:AddToggle("AimbotToggle", {
    ...
    Callback = function(val)
        ...
        if not val then
            ...
            if picker and picker.Mode == "Toggle" then
                aimbot.enabled = true
            end
            updaimbot()
        end
...
aimbotTab:AddToggle("AimbotVisibleOnly", {
    Text = "僅可見目標",
    Default = false,
    Callback = function(val)
        aimbot.visibleOnly = val
    end
})
...
aimbotTab:AddDropdown("AimbotCurve", {
    Text = "瞄準曲線",
    Default = "Linear",
...
aimbotCustomTab:AddSlider("AimbotFOVSpinSpd", {
    Text="旋轉速度", Default=1, Min=0.1, Max=10, Rounding=1, Compact=true,
    Callback=function(val) aimbotFOVCfg.SpinSpd=val end
})
aimbotCustomTab:AddToggle("AimbotFOVFollowMuzzle", {
    Text="跟隨槍口", Default=false,
...
local hitEffectsTab = HitGroup:AddTab('命中特效')
...
local hitEffectsToggle = hitEffectsTab:AddToggle("HitEffects", {
    Text = "啟用",
    Default = false,
    Callback = function(val)
...
hitEffectsTab:AddDropdown("HitEffectStyleDropdown", {
    Text = "樣式",
    Default = {"Particles"},
    Values = {
...
do
local hitSoundsTab = HitGroup:AddTab('命中音效')
...
hitSoundsTab:AddToggle("HitSounds", {
    Text = "啟用",
    Default = false,
    Callback = function(val)
        hitSoundEnabled = val
...
local soundDropdown
hitSoundsTab:AddInput("SoundSearch", {
    Text = "搜尋音效",
    Placeholder = "",
...
soundDropdown = hitSoundsTab:AddDropdown("SoundStyleDropdown", {
    Text = "音效",
    Default = "Rust HS",
    Values = allSounds,
    Callback = function(val)
...
hitSoundsTab:AddSlider("SoundVolume", {
    Text = "音量",
    Default = 50,
    Min = 1,
    Max = 100,
...
hitSoundsTab:AddSlider("SoundPitch", {
    Text = "音高",
    Default = 100,
    Min = 50,
    Max = 200,
...
hitSoundsTab:AddToggle("DisableGunSounds", {
    Text = "關閉槍械音效",
...
local vmTab = ViewModelTabbox:AddTab("視角模型")
local viewportTab = ViewModelTabbox:AddTab("視口")
...
local function getModelYBounds(model)
...
local function getFilteredModelYBounds(model, partFilter)
    local minY, maxY = math.huge, -math.huge
    for _, d in ipairs(model:GetDescendants()) do
        if d:IsA("BasePart") and partFilter(d) then
            minY = math.min(minY, d.Position.Y)
            maxY = math.max(maxY, d.Position.Y)
        end
    end
...
local function verticalChamColor(part, model, topColor, bottomColor, yBounds)
    local minY, maxY = yBounds[1], yBounds[2]
    local range = math.max(maxY - minY, 0.001)
    local t = math.clamp((part.Position.Y - minY) / range, 0, 1)
...
local function syncChamOutlineGlow(part, color, strength)
    if not part or not part.Parent then
        clearChamOutlineGlow(part)
        return
    end
    local s = math.clamp((tonumber(strength) or 5) / 10, 0, 1)
    local innerScale = 1.02 + s * 0.07
    local bloomScale = 1.06 + s * 0.2
    local innerTrans = math.clamp(0.55 - s * 0.4, 0.05, 0.95)
    local bloomTrans = math.clamp(0.88 - s * 0.55, 0.08, 0.95)
    local innerPad = Vector3.new(0.008, 0.008, 0.008) + Vector3.new(0.02, 0.02, 0.02) * s
    local bloomPad = Vector3.new(0.02, 0.02, 0.02) + Vector3.new(0.06, 0.06, 0.06) * s
    buildChamOutlineGlow(part, part, chamOutlineGlowClones, "ChamOutlineGlow", innerScale, innerTrans, innerPad)
    buildChamOutlineGlow(part, part, chamOutlineBloomClones, "ChamOutlineBloom", bloomScale, bloomTrans, bloomPad)
    local inner = chamOutlineGlowClones[part]
    local bloom = chamOutlineBloomClones[part]
    if inner then inner.Color = color end
    if bloom then bloom.Color = color end
end
...
viewportTab:AddToggle("EnableViewport", {Text = "啟用", Default = false, Callback = function(v)
    viewportEnabled = v
    if v then
        viewportloop()
    else
...
viewportTab:AddSlider("OffsetZ", {Text = "Z", Default = 0, Min = -10, Max = 10, Rounding = 2, Compact = true, Callback = function(v) offsetZ = v end})
...
local noAnimToggle = viewportTab:AddToggle("NoAnimations", {
    Text = "無動畫",
    Default = false,
    Callback = function(v)
...
local gunChamsToggle = vmTab:AddToggle("GunChams", {
    Text = "槍械透視",
    Default = false,
    Callback = function(v)
        gunChamsEnabled = v
...
local armChamsToggle = vmTab:AddToggle("ArmChams", {
    Text = "手臂透視",
    Default = false,
    Callback = function(v)
        armChamsEnabled = v
...
local gunHighlightToggle = vmTab:AddToggle("GunHighlight", {
    Text = "槍械高亮",
    Default = false,
    Callback = function(v)
...
local armHighlightToggle = vmTab:AddToggle("ArmHighlight", {
    Text = "手臂高亮",
    Default = false,
    Callback = function(v)
...
local function getEspPlayerWeapon(player)
    if not player then return "無" end
    local viewModels = workspace:FindFirstChild("ViewModels")
    if not viewModels then return "無" end
...
local rageLockStickyUntil = 0
local rageLockPrefer = "cursor" -- cursor | distance | health
local function nearest()
    ...
    if IS_MOBILE then
        if MOBILE_TOUCH_STATE.touching then
            ...
            local cam = workspace.CurrentCamera
            cursorpos = cam and Vector2.new(cam.ViewportSize.X / 2, cam.ViewportSize.Y / 2) or Vector2.zero
        end
    else
    ...
    -- 黏滯：未到期且舊目標仍有效 → 繼續用
    if tick() < rageLockStickyUntil and config.target.character and valid(config.target.character) then
        ...
        if root then
            ...
            local sp, on = wts(root.Position, cam)
            if on and sp and (Vector2.new(sp.X, sp.Y) - cursorpos).Magnitude <= maxFov * 1.25 then
                return config.target.character
            end
        end
    end
    ...
    for _, targetplayer in players:GetPlayers() do
        if targetplayer ~= player and targetplayer.Character then
            ...
            if valid(char) then
                local skip = false
                pcall(function()
                    if targetplayer:GetAttribute("InSpawn") or targetplayer:GetAttribute("InLobby")
                        or targetplayer:GetAttribute("InSafeZone") or targetplayer:GetAttribute("IsSpectating")
                        or targetplayer:GetAttribute("InIntermission") then
                        skip = true
                    end
                end)
                if not skip then
                    ...
                    if root and hum then
                        local wts = getgenv().InstanceWorldToScreen or worldToScreen
                        local screenpos, onscreen = wts(root.Position, cam)
                        if onscreen and screenpos then
                            ...
                            if screenDist <= maxFov then
                                local worldDist = myPos and (root.Position - myPos).Magnitude or 0
                                local score
                                if rageLockPrefer == "distance" then
                                    score = worldDist + screenDist * 0.05
                                elseif rageLockPrefer == "health" then
                                    score = hum.Health + screenDist * 0.1
                                else
                                    score = screenDist + worldDist * 0.012
                                end
                                pcall(function()
                                    local live = workspace:FindFirstChild("Live")
                                    if live and (char.Parent == live or char:IsDescendantOf(live)) then
                                        score = score - 35
                                    end
                                    if inLive and inLive(targetplayer) then score = score - 25 end
                                end)
                                if score < bestScore then
                                    besttarget, bestScore = char, score
                                end
                            end
                        end
                    end
...
local function hitpartfromname(character, partname)
    ...
    if partname == "Closest" then
        ...
        return closest or findPart("Head")
    elseif partname == "Random" then
        local parts = {"Head", "HumanoidRootPart", "UpperTorso", "Torso"}
        local p = character:FindFirstChild(parts[math.random(#parts)])
        return p or findPart("Head")
    else
...
local function atkCfg()
    ...
    local mode = config.target.attackPosition or "預設"
...
local function settarget(char)
    ...
    config.target.immune = false
    rageLockStickyUntil = tick() + 0.85 -- 黏滯，減少狂暴亂換人
    indicator.Color = Color3.fromRGB(255, 50, 50)
    local root = char:FindFirstChild("HumanoidRootPart")
...
local function nearplr()
    ...
    for _, p in ipairs(players:GetPlayers()) do
        ...
        local head = p.Character:FindFirstChild("HitboxHead") or p.Character:FindFirstChild("頭部")
...
runsvc.Heartbeat:Connect(function()
    ...
    if config.target.auto and not config.target.manualkey then
        ...
        if hasammo and not config.state.reloading then
            local needNew = (not config.target.enabled or not config.target.character or not valid(config.target.character))
            -- 已有目標時，若出現明顯更近的敵人則切換（避免鎖錯人一直不換）
            if not needNew and config.target.character then
                local better = nearest()
                if better and better ~= config.target.character then
                    local wts = getgenv().InstanceWorldToScreen or worldToScreen
                    local curRoot = config.target.character:FindFirstChild("HumanoidRootPart")
                    local newRoot = better:FindFirstChild("HumanoidRootPart")
                    if curRoot and newRoot then
                        local cursorpos = userinput:GetMouseLocation()
                        if IS_MOBILE then
                            local cam = workspace.CurrentCamera
                            cursorpos = cam and (cam.ViewportSize / 2) or cursorpos
                        end
                        local sp1, ok1 = wts(curRoot.Position, camera)
                        local sp2, ok2 = wts(newRoot.Position, camera)
                        if ok1 and ok2 and sp1 and sp2 then
                            local d1 = (Vector2.new(sp1.X, sp1.Y) - cursorpos).Magnitude
                            local d2 = (Vector2.new(sp2.X, sp2.Y) - cursorpos).Magnitude
                            -- 當前目標已離開合理 FOV → 強制換人
                            if d1 > 220 then
                                needNew = true
                            -- 新目標明顯更近才切換，避免抖動
                            elseif d2 < d1 * 0.55 and d2 < 180 then
                                needNew = true
                            end
                        elseif not ok1 then
                            -- 當前目標不在螢幕上 → 換人
...
rageui.ragebottab = rageui.targetgroup:AddTab('狂暴自瞄')
rageui.ragebotvisualtab = rageui.targetgroup:AddTab('視覺化')
...
rageui.ragebottab:AddDropdown("RageLockPrefer", {
    Text = "鎖定優先",
    Default = "準星",
    Values = { "準星", "距離", "血量" },
    Callback = function(val)
        if val == "距離" then rageLockPrefer = "distance"
        elseif val == "血量" then rageLockPrefer = "health"
        else rageLockPrefer = "cursor" end
    end
})
...
task.spawn(function()
    while true do
        ...
        if config and config.profile and config.profile.level and config.profile.level.enabled then
            player:SetAttribute("等級", config.profile.level.value)
        end
...
local nnTab = FrameScriptsufhner:AddTab("名稱")
local skinTab = FrameScriptsufhner:AddTab("皮膚")
...
local function applyskin(c)
    ...
    pcall(function()
        ...
        local head = c:FindFirstChild("頭部")
        if head then
...
nnTab:AddToggle("NameSpoofVerified", {
    Text = "已驗證",
    Default = false,
    Callback = function(val)
...
nnTab:AddToggle("NameSpoofPremium", {
    Text = "進階",
    Default = false,
    Callback = function(val)
...
local FrameScriptStats = Tabs.Character:AddLeftTabbox()
local fpsTab = FrameScriptStats:AddTab("FPS")
local msTab = FrameScriptStats:AddTab("延遲")
local regionTab = FrameScriptStats:AddTab("地區")
...
if not config.profile.regionspoof then
    config.profile.regionspoof = { enabled = false, value = "lunaragoat" }
end
    ...
    if config.profile.msspoof.fraud then
        ...
        task.spawn(function()
            while _G.MSFraudThread do
                ...
                for i = 1, #foundLabels do
                    local label = foundLabels[i]
                    local spoofed = label.Text:gsub("%d+ms", target .. "延遲")
                    if spoofed ~= label.Text then
                        label.Text = spoofed
                    end
                end
...
fpsTab:AddToggle("FPSSpoofEnabled", {
    Text = "啟用",
    Default = false,
    Callback = function(val)
...
fpsTab:AddToggle("FPSSpoofFraud", {
    Text = "詐欺",
    Default = false,
    Callback = function(val)
...
msTab:AddToggle("MSSpoofEnabled", {
    Text = "啟用",
    Default = false,
    Callback = function(val)
...
regionTab:AddToggle("RegionSpoofEnabled", {
    Text = "啟用",
    Default = false,
    Callback = function(val)
...
regionTab:AddInput("RegionSpoofValue", {
    ...
    Callback = function(val)
        config.profile.regionspoof.value = val or "lunaragoat"
...
v2:AddToggle("cframespf_enabled", {
    ...
    Callback = function(state)
        ...
        if state then
            ...
            velocityConn = RS.Heartbeat:Connect(function()
...
v2:AddToggle("cframefly_enabled", {
    ...
    Callback = function(state)
        ...
        flyConn = RS.Heartbeat:Connect(function()
            ...
            local char = LP.Character
            if not char or not char.Parent or not hrp or not hrp.Parent then cleanupFly(); return end
            ...
            if humCurrent then
                ...
                if humCurrent:GetState() ~= Enum.HumanoidStateType.PlatformStanding then
                    humCurrent:ChangeState(Enum.HumanoidStateType.PlatformStanding)
                end
            end
        end)
    end
...
v2:AddToggle("CHAR_SLIDE_BOOST", {
    ...
    Callback = function(Value)
        ...
        if Value then
            ...
            slideBoostConn = RS.Heartbeat:Connect(function()
                ...
                local hum = char:FindFirstChildOfClass("Humanoid")
                if not hrp or not hum then return end
                local isSliding = UIS:IsKeyDown(Enum.KeyCode.LeftControl)
                    or UIS:IsKeyDown(Enum.KeyCode.ButtonR3)
                if isSliding then
                    local moveDir = hum.MoveDirection
                    if moveDir.Magnitude > 0 then
...
v2:AddSlider("CHAR_SLIDE_BOOST_SPEED", {
    Text = "滑行加速速度",
    Default = 45,
    Min = 1,
    Max = 200,
...
local slfMtrlTab = characterSection:AddTab("自身材質")
local animPlayerTab = characterSection:AddTab("動畫播放器")
local SlfMtrl = {
    enabled = false,
...
slfMtrlTab:AddToggle("SlfMtrlEnable", {
    Text = "啟用",
    Default = false,
    Callback = function(val)
...
slfMtrlTab:AddDropdown("SlfMtrlMaterial", {
    Text = "材質",
...
slfMtrlTab:AddSlider("SlfMtrlTransparency", {
    Text = "透明度",
...
slfMtrlTab:AddSlider("SlfMtrlPulseSpeed", {
    Text = "脈衝速度",
...
local function updantiaim(deltatime)
    ...
    if settings.yawtype == "無" and settings.pitchtype == "無" and settings.angletype == "無" then
        return
    end
    local curweap = curweap2()
...
getgenv().InstanceSyncAfterConfigLoad = function()
    ...
    if Options and Options.AntiAimYaw then
        settings.yawtype = Options.AntiAimYaw.Value or "無"
    end
    if Options and Options.AntiAimPitch then
        settings.pitchtype = Options.AntiAimPitch.Value or "無"
    end
    if Options and Options.AntiAimAngle then
        settings.angletype = Options.AntiAimAngle.Value or "無"
    end
    if not settings.enabled or (settings.yawtype == "無" and settings.pitchtype == "無" and settings.angletype == "無") then
        flushAntiAimMovementState()
        underground_oldpos = nil
    end
end
...
antiaimbox:AddDropdown("AntiAimYaw", {
    Values = {"無", "jitter", "spinbot", "random"},
    Default = "None",
    Text = "偏航",
    Callback = function(value)
...
antiaimbox:AddDropdown("AntiAimAngle", {
    Values = {"無", "tilt 45", "tilt 90", "upside down", "custom"},
    Default = "None",
    Text = "角度",
    Callback = function(value)
...
MiscUnlockGroup:AddToggle("ENABLE_UNLOCK_ALL", {
    ...
    Callback = function(Value)
        if Value and not unlockAllExecuted then
            ...
            if Toggles.ENABLE_UNLOCK_ALL then
...
v555:AddToggle('handcaps', {
    Text = '啟用限制',
    Default = _G.Features.Handicaps.Enabled,
    Callback = function(val)
...
local function hideCharacterHead(char)
    if not char then return end
    local head = char:FindFirstChild("頭部")
...
local hitNotifTab = HitGroup:AddTab('命中通知')
local rbHitCfg = ragebot.config.hitNotifications
local hitNotificationsToggle = hitNotifTab:AddToggle('HitNotifications', {
    Text = '命中通知',
    Default = false,
    Callback = function(val)
...
local deviceBox = Tabs.Misc:AddRightGroupbox('Device Spoofer')
...
local slideBox = Tabs.Misc:AddRightGroupbox('滑行加速')
...
runsvc2.Heartbeat:Connect(function()
    ...
    if _G.Features.QOL.MotionBlur then
...
function applyultramodaurashadersminecraft()
    savelightoriginals()
    pcall(function()
    ...
    workspace.DescendantAdded:Connect(function(v)
        if _G.Features.QOL.Shaders and v:IsA("BasePart") then
            pcall(function() v.CastShadow = true end)
        end
    end)
end
...
task.spawn(function()
    ...
    RunService:BindToRenderStep('QOL_SmoothGameplay', Enum.RenderPriority.Input.Value, function()
        ...
        if _G.Features.QOL.SmoothGameplay then
            if avgFps < 45 then
    ...
    Players.PlayerRemoving:Connect(function(p)
        if p == LocalPlayer then
            ...
            if _G.Features.QOL.Shaders then removeshaders() end
        end
    end)
end)
;(function()
...
local function CreateBox(player)
    ...
    local studsgui = Instance.new("ScreenGui", game.CoreGui)
    local namegui = Instance.new("ScreenGui", game.CoreGui)
...
local colorTab = fxTabbox:AddTab('顏色')
local bloomTab = fxTabbox:AddTab('光暈')
colorTab:AddToggle('lighting_color_correction', {
    Text = '啟用',
...
colorTab:AddSlider('lighting_cc_contrast', {
    Text = '對比',
    Min = -10,
    Max = 10,
...
colorTab:AddSlider('lighting_cc_saturation', {
    Text = '飽和度',
    Min = -10,
    Max = 10,
...
colorTab:AddSlider('lighting_cc_brightness', {
    Text = '亮度',
    Min = -10,
    Max = 10,
...
bloomTab:AddToggle('lighting_bloom_modifier', {
    Text = '啟用',
...
bloomTab:AddSlider('lighting_bloom_multiplier', {
    Text = '發光強度',
    Min = 0,
    Max = 1.5,
...
bloomTab:AddSlider('lighting_bloom_size', {
    Text = '發光大小',
    Min = 2,
    Max = 8,
    Default = 4,
...
bloomTab:AddSlider('lighting_bloom_threshold', {
    Text = '亮度閾值',
    Min = 0.5,
    Max = 1,
    Default = 0.7,
...
Run.PostSimulation:Connect(function()
    ...
    local pos
    if cfg.crosshair_mode == "跟隨槍口" then
...
local atmotab = atmotabbox:AddTab('大氣')
local ambienceTab   = atmotabbox:AddTab('環境光')
...
local function playambientsound()
    ...
    curambientsound = Instance.new("音效")
...
atmotab:AddToggle('atmosphere_master', {
    Text = '啟用',
...
atmotab:AddSlider('atmosphere_density', {
    Text = '密度：',
    Min = 0,
    Max = 1,
...
atmotab:AddSlider('atmosphere_haze', {
    Text = '霧霾：',
    Min = 0,
    Max = 5,
...
atmotab:AddSlider('atmosphere_glare', {
    Text = '眩光：',
    Min = 0,
    Max = 5,
...
atmotab:AddSlider('atmosphere_offset', {
    Text = '偏移：',
    Min = 0,
    Max = 1,
...
ambienceTab:AddToggle('atmosphere_sound_toggle', {
    Text = '啟用',
...
ambienceTab:AddDropdown('atmosphere_ambient_sound', {
    Text = '環境音效',
    Values = {"無", "Rain", "Night", "Birds Chirping", "Jungle Birds", "Campfire Crackling", "Night Crickets", "Snow Storm"},
...
ambienceTab:AddSlider('atmosphere_sound_volume', {
    Text = '音量',
    Min = 0,
    Max = 10,
...
local skyTab      = skyboxTabbox:AddTab('天空盒')
local aspectTab   = skyboxTabbox:AddTab('長寬比')
...
skyTab:AddToggle('skybox_master', {
    Text = '啟用',
...
skyTab:AddDropdown('skybox_selection', {
    Text = '天空盒',
...
skyTab:AddDropdown('disable_elements', {
    Text = '停用元素',
...
aspectTab:AddToggle('aspect_ratio_master', {
    Text = '啟用',
...
aspectTab:AddSlider('aspect_ratio_x', {
    Text = 'X 比例',
...
aspectTab:AddSlider('aspect_ratio_y', {
    Text = 'Y 比例',
...
local function kbCalcPanelWidth(rows)
    local w = kbTextWidth("按鍵綁定", 14) + KB_PAD * 2 + 8
...
MenuGroup = Tabs['介面設定']:AddLeftGroupbox('menu settings')
...
MenuGroup:AddLabel('河馬科技'):AddKeyPicker('MenuKeybind', { 
    Default = 'RightShift', 
    NoUI = true, 
    Text = '河馬科技' 
})
...
(function()
local ServerGroup = Tabs['介面設定']:AddRightGroupbox('server')
local HttpService = game:GetService("HttpService")
...
local characterSection = Tabs.Character:AddRightTabbox()
local p = game:GetService("Players").LocalPlayer
local workspace = game:GetService("Workspace")
local runService = game:GetService("RunService")
getgenv()._animCmd = getgenv()._animCmd or nil
local animPresets = {
    ["冥想"] = "96579993895076", ["Orbit"] = "133811691098518", ["Orbit V2"] = "73208745607037",
    ["Orbit V3"] = "137869087140582", ["Floss"] = "72174079036035", ["OJ"] = "110064349530772",
    ["Kicking Feet"] = "131879764029003", ["Tweaking"] = "114353590132838", ["瘋狂"] = "120819498172771",
...
local animPresetNames = {
    "冥想", "Orbit", "Orbit V2", "Orbit V3", "Floss", "OJ", "Kicking Feet", "Tweaking", "瘋狂",
    "Crazy V2", "Take the L", "Hype", "Small / Hide", "Sea Lion", "Gangnam", "NLE", "The Man",
    "Spider Shit", "Low Cortisol", "Rampage",
}
animPlayerTab:AddToggle("AnimEnabled", {
    Text = "已啟用",
    Default = false,
}):AddKeyPicker("AnimKey", { Default = "None", SyncToggleState = true, Mode = "Toggle", Text = "動畫" })
animPlayerTab:AddToggle("AnimServerVisible", {
    Text = "伺服器端",
    Default = true,
})
animPlayerTab:AddToggle("AnimJitter", { Text = "抖動模式", Default = false })
animPlayerTab:AddSlider("JitterSpeed", { Text = "抖動間隔", Default = 0.1, Min = 0.01, Max = 2, Rounding = 2, Suffix = "s" })
animPlayerTab:AddToggle("AnimLoop", { Text = "循環", Default = true })
animPlayerTab:AddToggle("AnimAutoRespawn", { Text = "重生保護", Default = true })
animPlayerTab:AddInput("AnimForceID", { Default = "96579993895076", Text = "主要動畫 ID", Placeholder = "numbers", Numeric = false, Finished = true })
animPlayerTab:AddInput("AnimJitterID", { Default = "120819498172771", Text = "抖動動畫 ID", Placeholder = "numbers", Numeric = false, Finished = true })
animPlayerTab:AddSlider("AnimSpeed", { Text = "播放速度", Default = 2, Min = 0.1, Max = 200, Rounding = 1 })
animPlayerTab:AddDropdown("AnimPresetSelector", {
    Text = "主要預設",
    Values = animPresetNames,
    Default = "冥想",
})
animPlayerTab:AddDropdown("AnimJitterSelector", {
    Text = "抖動預設",
    Values = animPresetNames,
    Default = "瘋狂",
})
...
smoothTexSaStore.Parent = game.CoreGui
...
local function recordTargetTexture(inst)
    ...
    targetTextureOrig[inst] = {
        ...
        Transparency = getInstProperty(inst, "透明度"),
    }
end
local function restoreTargetTexture(inst)
    ...
    if inst:IsA("Texture") or inst:IsA("Decal") or inst:IsA("SurfaceAppearance") then
        ...
        if orig.Transparency ~= nil then setInstProperty(inst, "透明度", orig.Transparency) end
    end
...
local function isWorldPart(inst)
    ...
    if model then
        ...
        if model.Name:find("武器") or model.Name:find("Projectile") or model.Name:find("Effect") then return false end
    end
...
function ThemeManager:ThemeUpdate()
    ...
    if getgenv().InstanceApplyAccentTheme then
        pcall(getgenv().InstanceApplyAccentTheme)
    end
...
local MenuGroup = Tabs["介面設定"]:AddLeftGroupbox("Menu")
MenuGroup:AddToggle("KeybindMenuOpen", { Default = Library.KeybindFrame.Visible, Text = "開啟按鍵選單", Callback = function(value) Library.KeybindFrame.Visible = value end})
MenuGroup:AddToggle("ShowCustomCursor", {Text = "自訂游標", Default = true, Callback = function(Value) Library.ShowCustomCursor = Value end})
MenuGroup:AddDivider()
MenuGroup:AddLabel("河馬科技"):AddKeyPicker("MenuKeybind", { Default = "RightShift", NoUI = true, Text = "河馬科技" })
MenuGroup:AddToggle("HemaGlassUI", {
    Text = "動態玻璃 UI",
    Default = false,
    Callback = function(val)
        if getgenv().HemaGlassSetEnabled then
            getgenv().HemaGlassSetEnabled(val)
        else
            getgenv().HemaGlassEnabled = val
        end
    end
})
MenuGroup:AddSlider("HemaGlassStrength", {
    Text = "玻璃強度",
    Default = 70,
    Min = 25,
    Max = 100,
    ...
    Suffix = "%",
    Callback = function(val)
        if getgenv().HemaGlassSetStrength then
            getgenv().HemaGlassSetStrength((tonumber(val) or 70) / 100)
        end
    end
})
MenuGroup:AddButton("重設玻璃 UI", function()
    if getgenv().HemaGlassSetEnabled then
        getgenv().HemaGlassSetEnabled(false)
        task.wait(0.05)
        if Toggles and Toggles.HemaGlassUI then
            pcall(function() Toggles.HemaGlassUI:SetValue(false) end)
        end
    end
end)
MenuGroup:AddButton("卸載", function() Library:Unload() end)
Library.ToggleKeybind = Options.MenuKeybind 
ThemeManager:SetLibrary(Library)
SaveManager:SetLibrary(Library)
SaveManager:IgnoreThemeSettings()
SaveManager:SetIgnoreIndexes({ "MenuKeybind" })
ThemeManager:SetFolder("河馬科技")
SaveManager:SetFolder("河馬科技/specific-game")
SaveManager:BuildConfigSection(Tabs["介面設定"])
ThemeManager:ApplyToTab(Tabs["介面設定"])
